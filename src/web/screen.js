...
export function OsanwallCanvasApp() {
    ...
    return (<div className="appRoot">
      <div className="canvasWrap" onWheel={(e) => {
        ...
        <div className="overlay">
          {motes.map((m) => (<motion.div key={m.id} className={`mote ${m.status}`} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} style={{
      ...
      <div className="composer">
        ...
        <textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Drop a new mote at current viewport..."/>
        <button onClick={createMote}>Drop Mote</button>
        <label className="toggleRow">
          <input type="checkbox" checked={settings.muted} onChange={(e) => setSettings({ ...settings, muted: e.target.checked })}/>
          Mute notifications
        </label>
